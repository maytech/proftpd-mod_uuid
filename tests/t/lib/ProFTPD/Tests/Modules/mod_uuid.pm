package ProFTPD::Tests::Modules::mod_uuid;
...
sub uuid_ok {
  ...
  my $config_file = "$tmpdir/uuid.conf";
  my $pid_file = File::Spec->rel2abs("$tmpdir/uuid.pid");
  my $scoreboard_file = File::Spec->rel2abs("$tmpdir/uuid.scoreboard");

  my $log_file = File::Spec->rel2abs('tests.log');

  my $auth_user_file = File::Spec->rel2abs("$tmpdir/uuid.passwd");
  my $auth_group_file = File::Spec->rel2abs("$tmpdir/uuid.group");
